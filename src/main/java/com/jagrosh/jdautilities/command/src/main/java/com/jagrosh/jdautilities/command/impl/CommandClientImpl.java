 ...
 */
package com.jagrosh.jdautilities.command.src.main.java.com.jagrosh.jdautilities.command.impl;

import com.jagrosh.jdautilities.command.src.main.java.com.jagrosh.jdautilities.command.*;
import com.jagrosh.jdautilities.commons.src.main.java.com.jagrosh.jdautilities.commons.utils.FixedSizeCache;
import com.jagrosh.jdautilities.commons.src.main.java.com.jagrosh.jdautilities.commons.utils.SafeIdUtil;
...
public class CommandClientImpl implements CommandClient, EventListener
{
        ...
        this.helpConsumer = helpConsumer==null ? (event) -> {
                ...
                event.replyInDm(builder.toString(), unused ->
                {
                    if(event.isFromType(ChannelType.TEXT))
                        event.reactSuccess();
                }, t -> event.replyWarning("Sorry, I wasn't able to DM you help, cause you didnt open the door!\"" +
                        "(you're blocking DM's)"));
        } : helpConsumer;
    ...
    private void onReady(ReadyEvent event)
    {
        ...
        try {
            ...
            pref = Database.initPref(event.getJDA(), event.getJDA().getGuilds().get(0).getId());
            prefix = pref.getPrefix();
        }
            setGame(event);
    ...
    public void setGame(Event event){
        ...
        event.getJDA().getPresence().setPresence(OnlineStatus.ONLINE,
                Game.playing("Type @"+ event.getJDA().getSelfUser().getName()+" "+helpWord));
    ...
    private void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        if(rawContent.startsWith(event.getJDA().getSelfUser().getAsMention())
        )
        {
            parts = splitOnPrefixLength(rawContent, event.getJDA().getSelfUser().getAsMention().length()+1);
        }
